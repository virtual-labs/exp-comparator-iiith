    ...
    switch (this.type) {
      case "AND":
        this.output = computeAnd(
          getOutputFA(this.inputs[0], this.input_pos[0]),
          getOutputFA(this.inputs[1], this.input_pos[1])
        );
        break;
      case "OR":
      ...
      case "THREEIPNAND":
        this.output = computeNand(
          ...
          getOutputFA(this.inputs[2], this.input_pos[2])
        );
        break;
      case "NOR":
...
export function submitCircuit() {
  ...
  if (window.currentTab === "task1") {
    ...
    if (!checkConnectionsFA() || !checkConnections()) return;
    testComparator1(
      "Input-0",
      "Input-1",
      "Input-2",
...
export function deleteElement(gateid) {
  try {
    ...
    if (!element) {
      console.log("DOM element not found for gate:", gateid);
      delete gates[gateid];
      return;
    }
    ...
    // Clean up references in other gates
    ...
    delete gates[gateid];
    console.log("Gate deleted successfully:", gateid);
  } catch (error) {
